...
export ANCHOR_PEER=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/crypto-config/fabricTest/HLF/channel-artifacts/Org1MSPanchors.tx
...
kubectl exec $CLI_POD_ID -it -- bash -c "CORE_PEER_LOCALMSPID=$CORE_PEER_LOCALMSPID && CORE_PEER_MSPCONFIGPATH=$CORE_PEER_MSPCONFIGPATH && CORE_PEER_ADDRESS=$CORE_PEER_ADDRESS && peer channel update -o $ORDERER_ADDR -c $CHANNEL_NAME -f $ANCHOR_PEER"


#setting up the peer0org2 env variables inside cli environment
export ANCHOR_PEER=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/crypto-config/fabricTest/HLF/channel-artifacts/Org2MSPanchors.tx
export GENESIS_BLOCK=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/crypto-config/fabricTest/HLF/channel-artifacts/orderer-channel.tx
export CLI_POD_ID=`kubectl get pod | grep cli | cut -f1 -d' '`
export ORDERER_ADDR="dev-orderer-g746q:7050"
